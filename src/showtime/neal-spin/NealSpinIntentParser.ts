...
interface Segment {
  dtMs: number;
  dAngleDeg: number; // normalized to (-180, 180]
  moving: boolean;
}

interface ArcChain {
  coverageCwDeg: number;
  coverageCcwDeg: number;
  cwSegments: number;
  ccwSegments: number;
  maxBacktrackDeg: number; // vs. the chain's own dominant direction
  activeMotionMs: number;
}
...
function quantizeDelta(deg: number): number {
  return Math.round(deg * 1e6) / 1e6;
}

function toAngleDeg(sample: AttemptSample, center: { x: number; y: number }): number {
  return (Math.atan2(sample.y - center.y, sample.x - center.x) * 180) / Math.PI;
}

function buildSegments(trace: NealSpinAttemptTrace, config: NealSpinConfig): Segment[] {
  const segments: Segment[] = [];
  for (let i = 1; i < trace.samples.length; i++) {
    const dtMs = trace.samples[i].tMs - trace.samples[i - 1].tMs;
    const dAngleDeg = quantizeDelta(
      normalizeDelta(
        toAngleDeg(trace.samples[i], trace.center) - toAngleDeg(trace.samples[i - 1], trace.center),
      ),
    );
    segments.push({
      dtMs,
...
function chainDominant(chain: ArcChain): { direction: RotationDirection; ratio: number; coverageDeg: number } {
...
export function parseAttempt(trace: NealSpinAttemptTrace, config: NealSpinConfig): IntentDimensions {
  ...
  const chains: ArcChain[] = [];
  let current: ArcChain | null = null;
  let pauseRunMs = 0;
  let maxPauseMs = 0;
  ...
  let hesitation: HesitationObservation = {
    occurred: false,
    stopDurationMs: 0,
    coverageAtStopDeg: 0,
  ...
  // Per-chain backtrack tracking needs the chain's dominant direction, which
  // is only known at the end — so track the worst contiguous reversal against
  // BOTH directions and pick the one matching the final dominant direction.
  interface BacktrackTracker {
    runVsCw: number;
    runVsCcw: number;
    maxVsCw: number;
    maxVsCcw: number;
  }
  let backtrack: BacktrackTracker = { runVsCw: 0, runVsCcw: 0, maxVsCw: 0, maxVsCcw: 0 };
  const chainBacktracks: BacktrackTracker[] = [];
  ...
  let best: ArcChain | null = null;
  let bestBacktrack: BacktrackTracker | null = null;
  let bestCoverage = -1;
  ...
  let dominantDirection: RotationDirection = "ambiguous";
