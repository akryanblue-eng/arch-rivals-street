...
export function buildIntentScoreEvent(args: {
  sessionId: string;
  attemptNumber: number;
  inputMethod: InputMethod;
  dims: IntentDimensions;
  velocityFloorState: VelocityFloorState;
  buildMode: BuildMode;
}): EventInputIntentScore {
  const { dims } = args;
  return {
...
export function buildFailureEvent(args: {
  sessionId: string;
  attemptNumber: number;
  inputMethod: InputMethod;
  failureClass: NealSpinFailure;
}): EventFailureType {
  return {
...
export function buildSuccessEvent(args: {
  sessionId: string;
  attemptNumber: number;
  inputMethod: InputMethod;
}): EventNealSpinSuccess {
...
export function buildGhostOverlayShownEvent(args: {
  sessionId: string;
  attemptNumber: number;
  failureClass: "Fail_Timing_Early" | "Fail_Timing_Late";
}): EventGhostOverlayShown {
  return {
...
export function buildHesitationPromptShownEvent(args: {
  sessionId: string;
  promptText: string;
  activationCountThisSession: number;
}): EventHesitationPromptShown {
  return {
...
export function buildHesitationRecoveryEvent(args: {
  sessionId: string;
  timeToReengageMs: number;
}): EventHesitationRecovery {
  return {
...
export function buildHesitationRecoveryOutcomeEvent(args: {
  sessionId: string;
...
export function buildAbandonmentEvent(args: {
  sessionId: string;
  lastFailureClass: NealSpinFailure | null;
  attemptsThisSession: number;
  timeInTutorialMs: number;
}): EventAbandonment {
  return {
...
function finite(name: string, value: unknown, problems: string[]): void {
  if (typeof value !== "number" || !Number.isFinite(value)) {
    problems.push(`${name} must be a finite number, got ${String(value)}`);
  }
}

function nonEmptyString(name: string, value: unknown, problems: string[]): void {
  if (typeof value !== "string" || value.length === 0) {
    problems.push(`${name} must be a non-empty string`);
  }
}

export function validateEvent(event: NealSpinTelemetryEvent): string[] {
  ...
  switch (event.type) {
    ...
    case "Event_Failure_Type": {
      ...
      if (
        !NEAL_SPIN_RESULTS.includes(event.failure_class) ||
        (event.failure_class as string) === "Success" ||
        (event.failure_class as string) === "NealSpin_Fail"
      ) {
    ...
    case "Event_Hesitation_Recovery_Outcome":
      if (!NEAL_SPIN_RESULTS.includes(event.outcome)) {
