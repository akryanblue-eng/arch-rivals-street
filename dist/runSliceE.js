...
const AgentEvolutionProtocol_1 = require("./evolution/AgentEvolutionProtocol");
const evolutionReplayAudit_1 = require("./evolution/evolutionReplayAudit");
const AepDecisionLedger_1 = require("./evolution/AepDecisionLedger");
const AepPrincipleRegistry_1 = require("./evolution/AepPrincipleRegistry");
function run() {
    (0, AgentEvolutionProtocol_1.resetSession)();
    (0, evolutionReplayAudit_1.resetLedger)();
    (0, AepDecisionLedger_1.resetDecisionLedger)();
    (0, AepPrincipleRegistry_1.resetPrincipleRegistry)();
    const sliceOutcomes = [];
    ...
    const retrievalProposal = {
        proposalId: "prop-001",
        component: "RETRIEVAL",
        change: "Increase context window from 512 to 1024 tokens.",
        ledgerRef: "rig-failure-2026-07-08-001",
    };
    (0, evolutionReplayAudit_1.recordProposalCreated)(retrievalProposal);
    const retrievalDecision = (0, AgentEvolutionProtocol_1.propose)(retrievalProposal, () => ({
        baselineErrorRate: 0.60,
        candidateErrorRate: 0.00,
        harmful: false,
    }));
    const retrievalEvent = (0, evolutionReplayAudit_1.record)(retrievalDecision);
    console.log(`Case 1 — RETRIEVAL: ${retrievalDecision.outcome}` +
    ...
    (0, AepDecisionLedger_1.appendEntry)(retrievalDecision, "Retrieval precision drop causing downstream execution latency friction.", {
    ...
    const baseModelProposal = {
        proposalId: "prop-020",
        component: "BASE_MODEL",
        change: "Fine-tune on extended gameplay corpus.",
        ledgerRef: "rig-failure-2026-07-08-001",
    };
    (0, evolutionReplayAudit_1.recordProposalCreated)(baseModelProposal);
    const baseModelDecision = (0, AgentEvolutionProtocol_1.propose)(baseModelProposal, () => ({
        // This runner is never called: ladder check fires first.
        baselineErrorRate: 0.60,
        candidateErrorRate: 0.60,
        harmful: false,
    }));
    const baseModelEvent = (0, evolutionReplayAudit_1.record)(baseModelDecision);
    console.log(`Case 2 — BASE_MODEL: ${baseModelDecision.outcome}` +
    ...
    (0, AepDecisionLedger_1.appendEntry)(baseModelDecision, "Retrieval precision drop causing downstream execution latency friction.", {
        failure_class: "RETRIEVAL_PRECISION_DEGRADATION",
        affected_metric: "execution_latency",
    ...
    const toolPolicyProposal = {
        proposalId: "prop-030",
        component: "TOOL_POLICY",
        change: "Allow agent to invoke external write APIs without confirmation.",
        ledgerRef: "rig-failure-2026-07-08-002",
    };
    (0, evolutionReplayAudit_1.recordProposalCreated)(toolPolicyProposal);
    const toolPolicyDecision = (0, AgentEvolutionProtocol_1.propose)(toolPolicyProposal, () => ({
        baselineErrorRate: 0.60,
        candidateErrorRate: 0.80,
        harmful: true,
    }));
    const toolPolicyEvent = (0, evolutionReplayAudit_1.record)(toolPolicyDecision);
    // Explicit audit checkpoint: simulation gate blocked this deployment.
    (0, evolutionReplayAudit_1.recordSimulationEnforced)(toolPolicyProposal.proposalId, toolPolicyProposal.ledgerRef);
    console.log(`Case 3 — TOOL_POLICY: ${toolPolicyDecision.outcome}` +
    ...
    (0, AepDecisionLedger_1.appendEntry)(toolPolicyDecision, "Edge-case physics exceptions tracking drift during rapid movement updates.", {
    ...
    const historicalRestraintQuery = (0, AepDecisionLedger_1.queryPriorIntervention)({
    ...
    assert(historicalRestraintQuery.result === "PRIOR_FAILURE", "Case 4: memory query must return PRIOR_FAILURE for blocked tool policy");
    assert(historicalRestraintQuery.recommended_action === "DO_NOT_RETRY_WITHOUT_NEW_EVIDENCE", "Case 4: memory gate must recommend DO_NOT_RETRY_WITHOUT_NEW_EVIDENCE");
    sliceOutcomes.push({
    ...
    const novelExplorationQuery = (0, AepDecisionLedger_1.queryPriorIntervention)({
    ...
    assert(novelExplorationQuery.result === "NOVEL_INTERVENTION", "Case 5: memory query must return NOVEL_INTERVENTION for novel planner intervention");
    assert(novelExplorationQuery.recommended_action === "PROCEED_TO_SIMULATION", "Case 5: novel intervention must be cleared for PROCEED_TO_SIMULATION");
    ...
    (0, evolutionReplayAudit_1.recordProposalCreated)(plannerProposal);
    const plannerDecision = (0, AgentEvolutionProtocol_1.propose)(plannerProposal, () => ({
        baselineErrorRate: 0.45,
        candidateErrorRate: 0.10,
        harmful: false,
    }));
    const plannerEvent = (0, evolutionReplayAudit_1.record)(plannerDecision);
    console.log(`Case 5 — PLANNER: ${plannerDecision.outcome}` +
    ...
    (0, AepDecisionLedger_1.appendEntry)(plannerDecision, "Planner producing suboptimal execution order under resource contention.", {
    ...
    (0, evolutionReplayAudit_1.recordProposalCreated)(crossoverProposalLegacy);
    const crossoverDecisionLegacy = (0, AgentEvolutionProtocol_1.propose)(crossoverProposalLegacy, () => ({
        baselineErrorRate: 0.15,
        candidateErrorRate: 0.65,
        harmful: false,
    }));
    (0, evolutionReplayAudit_1.record)(crossoverDecisionLegacy);
    (0, evolutionReplayAudit_1.recordSimulationEnforced)(crossoverProposalLegacy.proposalId, crossoverProposalLegacy.ledgerRef);
    assert(crossoverDecisionLegacy.outcome === "BLOCKED", "Case 6a seed must be BLOCKED");
    const historicalBlockedEntry = (0, AepDecisionLedger_1.appendEntry)(crossoverDecisionLegacy, "Crossover physics float drift.", {
    ...
    const crossoverMem001Query = (0, AepDecisionLedger_1.queryPriorIntervention)({
        target_subsystem: "TOOL_POLICY",
    ...
    assert(crossoverMem001Query.result === "PRIOR_FAILURE", "Case 6b: AEP-MEM-001 must return PRIOR_FAILURE");
    assert(crossoverMem001Query.recommended_action === "DO_NOT_RETRY_WITHOUT_NEW_EVIDENCE", "Case 6b: AEP-MEM-001 must recommend DO_NOT_RETRY");
    // Step 6c: AEP-MEM-002 context check — same legacy context → HALTED_BY_MEMORY.
    const sameContextCheck = (0, AepDecisionLedger_1.evaluateContextDelta)({
        target_subsystem: "TOOL_POLICY",
        proposal_class: "micro-checks",
        current_context: {
            simulator_version: "v1.0.2-legacy",
    ...
    assert(sameContextCheck.outcome === "HALTED_BY_MEMORY", "Case 6c: identical context must return HALTED_BY_MEMORY");
    // AEP-MEM-002 context check — upgraded simulator context → VETO_EVAPORATED.
    const newContextCheck = (0, AepDecisionLedger_1.evaluateContextDelta)({
        target_subsystem: "TOOL_POLICY",
        proposal_class: "micro-checks",
        current_context: {
    ...
    assert(newContextCheck.outcome === "VETO_EVAPORATED", "Case 6c: changed context must return VETO_EVAPORATED");
    assert(newContextCheck.resolution_type === "SIMULATOR_INVALIDATION", "Case 6c: resolution type must be SIMULATOR_INVALIDATION");
    assert(newContextCheck.overruled_entry_id === historicalBlockedEntry.entry_id, "Case 6c: overruled_entry_id must reference the historical BLOCKED entry");
    assert(newContextCheck.context_delta?.simulator_version?.old === "v1.0.2-legacy", "Case 6c: delta must record old simulator version");
    assert(newContextCheck.context_delta?.simulator_version?.new === "v2.0.0-high-fidelity", "Case 6c: delta must record new simulator version");
    // Step 6d: Veto evaporated — proceed to ladder check and simulation.
    const crossoverProposalNew = {
        proposalId: "prop-060-new",
        component: "TOOL_POLICY",
        change: "Bypass high-frequency micro-checks on position update vectors.",
        ledgerRef: "rig-failure-2026-08-01-001",
    };
    (0, evolutionReplayAudit_1.recordProposalCreated)(crossoverProposalNew);
    const crossoverDecisionNew = (0, AgentEvolutionProtocol_1.propose)(crossoverProposalNew, () => ({
        baselineErrorRate: 0.45,
        candidateErrorRate: 0.05,
        harmful: false,
    }));
    const crossoverNewEvent = (0, evolutionReplayAudit_1.record)(crossoverDecisionNew);
    console.log(`Case 6d — TOOL_POLICY (high-fidelity simulator): ${crossoverDecisionNew.outcome}` +
    ...
    const promotedEntry6d = (0, AepDecisionLedger_1.appendEntry)(crossoverDecisionNew, "Crossover physics float drift.", {
        failure_class: "TOOL_POLICY_SIMULATION_ARTIFACT",
        affected_metric: "position_sync_error_rate",
    ...
    const tamperedResult = (0, evolutionReplayAudit_1.attemptDirectDeploy)("prop-adversarial-001");
    ...
    const integrityShield = (0, AepPrincipleRegistry_1.registerPrinciple)("LEVEL_0_DETERMINISM_INTEGRITY", "Coordinate alignment must not drift beyond epsilon at the position sync boundary.", historicalBlockedEntry.entry_id);
    const pathfindingHeuristic = (0, AepPrincipleRegistry_1.registerPrinciple)("LEVEL_3_BEHAVIORAL_OPTIMIZATION", "Choose aggressive shortcut path for offensive advantage.", promotedEntry6d.entry_id);
    const taxonomyConflictType = (0, AepPrincipleRegistry_1.detectConflict)(integrityShield, pathfindingHeuristic);
    console.log(`\nCase 7a — detectConflict (taxonomy): ${taxonomyConflictType}`);
    assert(taxonomyConflictType === "TAXONOMY_OVERRIDE", "Case 7a: different taxonomy levels must produce TAXONOMY_OVERRIDE");
    const taxonomyResolution = (0, AepPrincipleRegistry_1.resolveConflict)(integrityShield, pathfindingHeuristic, taxonomyConflictType);
    ...
    assert(pathfindingHeuristic.lifecycle_catalyst === "EMPIRICAL", "Case 7a: arbitration-driven SUBORDINATED transition must record EMPIRICAL catalyst");
    ...
    const broadMovementPolicy = (0, AepPrincipleRegistry_1.registerPrinciple)("LEVEL_3_BEHAVIORAL_OPTIMIZATION", "Optimize player movement efficiency.", promotedEntry6d.entry_id);
    const narrowReplayPolicy = (0, AepPrincipleRegistry_1.registerPrinciple)("LEVEL_3_BEHAVIORAL_OPTIMIZATION", "During frame reconciliation, preserve exact replay determinism at the position sync boundary.", historicalBlockedEntry.entry_id);
    const specificityConflictType = (0, AepPrincipleRegistry_1.detectConflict)(broadMovementPolicy, narrowReplayPolicy);
    console.log(`\nCase 7b — detectConflict (specificity): ${specificityConflictType}`);
    assert(specificityConflictType === "SPECIFICITY_COLLISION", "Case 7b: same taxonomy level with different scope must produce SPECIFICITY_COLLISION");
    const specificityResolution = (0, AepPrincipleRegistry_1.resolveConflict)(broadMovementPolicy, narrowReplayPolicy, specificityConflictType);
    ...
    assert(broadMovementPolicy.lifecycle_catalyst === "EMPIRICAL", "Case 7b: arbitration-driven SUBORDINATED transition must record EMPIRICAL catalyst");
    ...
    const tolerancePrincipleA = (0, AepPrincipleRegistry_1.registerPrinciple)("LEVEL_2_SIMULATION_ACCURACY", "Allow +0.003 coordinate tolerance during high-velocity interpolation.", historicalBlockedEntry.entry_id, 0.003);
    // Check before registering B: sum is 0.003, proposing 0.004 more → fits.
    const envelopeCheckBeforeB = (0, AepPrincipleRegistry_1.aggregateEnvelopeCheck)(0.004, (0, AepPrincipleRegistry_1.dumpPrincipleRegistry)());
    console.log(`\nCase 7c — envelope check (0.003 active + 0.004 proposed): ${envelopeCheckBeforeB}`);
    assert(envelopeCheckBeforeB === "WITHIN_ENVELOPE", "Case 7c: 0.003 + 0.004 must be WITHIN_ENVELOPE");
    const tolerancePrincipleB = (0, AepPrincipleRegistry_1.registerPrinciple)("LEVEL_2_SIMULATION_ACCURACY", "Allow +0.004 coordinate tolerance during crossover boundary transitions.", promotedEntry6d.entry_id, 0.004);
    ...
    assert(pathfindingHeuristic.lifecycle_catalyst === undefined, "Case 7d-1: reactivation must clear lifecycle_catalyst (ACTIVE has no catalyst)");
    ...
    assert(broadMovementPolicy.lifecycle_catalyst === "EMPIRICAL", "Case 7d-2: supersedePrinciple must record EMPIRICAL catalyst (algorithmic replacement)");
    assert(improvedMovementPolicy.supersedes === broadMovementPolicy.principle_id, "Case 7d-2: supersedes must reference the replaced principle for backward lineage traversal");
    ...
    assert(tolerancePrincipleA.lifecycle_catalyst === "ADMINISTRATIVE", "Case 7d-3: deprecatePrinciple must record ADMINISTRATIVE catalyst (maintainer policy action)");
    ...
    assert(narrowReplayPolicy.lifecycle_catalyst === "ADMINISTRATIVE", "Case 7d-4: retirePrinciple must record ADMINISTRATIVE catalyst (maintainer review action)");
    ...
    assert(evidence.audit.tampered_ledger_deploy_attempt === "DETECTED", "Evidence: tampered_ledger_deploy_attempt must be DETECTED");
    assert(evidence.audit.simulation_requirement === "ENFORCED", "Evidence: simulation_requirement must be ENFORCED");
    const events = (0, evolutionReplayAudit_1.dumpLedger)();
    ...
    const decisionLedgerEntries = (0, AepDecisionLedger_1.dumpDecisionLedger)();
    ...
    const principleEntries = (0, AepPrincipleRegistry_1.dumpPrincipleRegistry)();
    const resolutionEntries = (0, AepPrincipleRegistry_1.dumpResolutionLog)();
