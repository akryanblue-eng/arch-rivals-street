...
function classifyAttempt(dims, config, inputMethod) {
    // 1. Timing.
    if (dims.timingOffsetMs < 0)
        return "Fail_Timing_Early";
    if (dims.timingOffsetMs > 0)
        return "Fail_Timing_Late";
    ...
    const pathValid = dims.arcCoverageDeg >= config.minEffectiveRotationDeg &&
        dims.dominantDirection !== "ambiguous" &&
        dims.maxBacktrackDeg <= config.backtrackToleranceDeg;
    if (!pathValid)
        return "Fail_Dexterity_Path";
